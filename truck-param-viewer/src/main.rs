...
impl App for MyApp {
    fn init(window: Arc<winit::window::Window>) -> MyApp {
        ...
        let (instance, wireframe, rx, watcher) = if args.len() >= 2 {
            let file = &args[1];
            let path = PathBuf::from(file);
            let openfile = std::fs::File::open(&path).unwrap();
            let (instance, wireframe) = Self::load_shape(&creator, openfile);
            scene.add_object(&instance);
            scene.add_object(&wireframe);

            let (tx, rx) = channel();
            let mut watcher: RecommendedWatcher =
                notify::watcher(tx, std::time::Duration::from_secs(2)).unwrap();

            watcher
                .watch(path.as_path(), RecursiveMode::Recursive)
                .unwrap();

            (instance, wireframe, Some(rx), Some(watcher))
        } else {
            let mesh: PolygonMesh = Default::default();
            let instance = creator.create_instance(&mesh, &Default::default());
            let wireframe = creator.create_instance(&mesh, &Default::default());
            (instance, wireframe, None, None)
        };
